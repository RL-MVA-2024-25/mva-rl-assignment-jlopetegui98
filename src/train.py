...
def build_model(config):
    return torch.nn.Sequential(
        ...
        torch.nn.Linear(256, 256),
        torch.nn.ReLU(),
        torch.nn.Linear(256,config['nb_actions'])
    )
...
class ProjectAgent:
    ...
    def train(self, env, max_episode):
        ...
        best_reward = -np.inf
        last_update_best_reward = -np.inf
        step = 0
    ...
    def act(self, observation, use_random=False, epsilon=1.0):
        if use_random and np.random.rand() < epsilon:
    ...
    def load(self, path='my_best_agent.pth'):
...
if __name__ == '__main__':
    ...
    # Train the agent
    episode_return = agent.train(env, 300)
    # ## Save the agent
